    ...
    try {
      const getBooksByAuthor = new Promise((resolve, reject) => {
        ...
        bookKeys.forEach(key => {
          if (books[key].author === author) filteredBooks.push(books[key]);
        });
      ...
      res.status(404).json({message: "Error finding author"});
    }
  });
...
public_users.get('/title/:title', async function (req, res) {
    ...
    try {
      const getBooksByTitle = new Promise((resolve, reject) => {
        ...
        let filteredBooks = [];
        bookKeys.forEach(key => {
          if (books[key].title === title) filteredBooks.push(books[key]);
        });
        resolve(filteredBooks);
      });
      ...
      res.status(404).json({message: "Error finding title"});
    }
  });
